...
def load_models_and_tokenizer(base_model_name="google/gemma-3-270m-it", peft_model_path="./models/gemma3-270m-tool-calling-final"):
    """
...
def format_prompt_for_eval(sample):
    """
    Formats a sample into Gemma's chat template format.
    """
    query = sample['query']
    ...
    # Gemma-3 uses a different chat template format
    prompt = (
...
def main():
    ...
    peft_model_path = "./models/gemma3-270m-tool-calling-final"
    ...
    for i, sample in tqdm(enumerate(eval_dataset), desc="Metrics (Fine-Tuned)", total=len(eval_dataset)):
        ...
        # Split on Gemma's model turn marker
        ft_pred_str = ft_raw_output.split('<start_of_turn>model\n')[1].strip() if '<start_of_turn>model' in ft_raw_output else ""
